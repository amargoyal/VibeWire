...
- Dark-only is a product decision, so contrast is carried entirely by the dark palette and must be verified there.
- Dynamic Type, VoiceOver, and Reduce Motion support are **not** yet established. Reduce Motion in particular is a live gap given the ambient animations. Treat as an open decision, not as satisfied.
