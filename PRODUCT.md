...
- **Dynamic Type is the live gap, and it is on the phone.** The browser scales its whole type ramp off the reader's root size and its rows grow rather than crop. The phone's remote screen documents a clamp at `accessibility1` that does not bind, because `NS.Font` resolves through `UIFontMetrics` against the application's content size category rather than SwiftUI's environment — so every mono readout on that screen scales past the limit the comment describes. Treat as open, not as satisfied.
