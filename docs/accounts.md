...
Mac setup requires a verified account as its final step. Signing out from Settings unpairs every device and returns the Mac to the sign-in screen. The host remembers completion, so normal launches remain available offline. Remote browser account enforcement is a separate setting. The email-code flow below serves both sign-in and account creation.
