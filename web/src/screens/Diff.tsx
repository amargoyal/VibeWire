 ...
 *
 * Past 900px the 560px column comes off and the listing takes the window. Width is
 * the one thing a diff wants — it is the whole reason the listing does not wrap —
 * and nothing is added beside it, because the only other things on this screen are
 * a header and a footer and neither gets better at 1280px.
 *
 * Two marks answer the extra width by themselves, which is why there is no code
 * here for either: the path truncates from the head only while it overflows, so it
 * stops; and the reach bar is drawn from a measured fraction of the widest line, so
 * it grows and then goes when nothing is off the glass any more.
 */
